...
def test_build_scheduler_creates_asyncio_scheduler_and_cron_job():
    ...
    scheduler = build_scheduler("30 18 * * 1-5", digest_job, scheduler_timezone=timezone.utc)
...
def test_build_scheduler_mon_to_fri_cron_triggers_on_monday():
    ...
    assert next_fire == datetime(2026, 4, 13, 18, 30, tzinfo=timezone.utc)
...
def test_build_digest_scheduler_registers_weekly_job_for_sunday_evening():
    daily_job = DigestJob(
        pipeline=_FakePipeline([]),
        digest_request=DigestRequest(
            kind="daily",
    ...
    weekly_scheduler_job = next(job for job in scheduler.get_jobs() if job.func == weekly_job.run)

    next_fire = weekly_scheduler_job.trigger.get_next_fire_time(
        previous_fire_time=None,
        now=datetime(2026, 4, 19, 18, 0, tzinfo=timezone.utc),
    )

    assert next_fire == datetime(2026, 4, 19, 18, 30, tzinfo=timezone.utc)
